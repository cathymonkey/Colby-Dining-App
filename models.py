...
class Student(db.Model):
    ...
    fav = db.relationship('Favorites', backref = 'stu')
...
class Favorites(db.Model):
    ...
    student_email = db.Column(db.String(255), db.ForeignKey('stu.student_email'), nullable = False)
    food_id = db.Column(db.Integer, db.ForeignKey('food.id'), nullable = False)

class Administrator(db.Model):
    admin_email = db.Column(db.String(255), primary_key = True)
    password_hashed = db.Column(db.String(128), nullable = False)
    feedback = db.relationship('Feedback', backref = 'admin')

class Feedback(db.Model):
    id = db.Column(db.Integer, primary_key = True)
    admin_email = db.Column(db.String(255), db.ForeignKey('admin.admin_email'), nullable = False)
